...
function getLineCount(SplFileObject $file) {
...
while (! $input->eof()) {
    if ($lineNumber === $lineCount) {
        printf('Program terminated successfully.'.PHP_EOL);

        break;
    }

    if (in_array($lineNumber, $ranLines, true)) {
        printf('Line %d has already run!'.PHP_EOL, $lineNumber + 1);

        break;
    }

    $input->seek($lineNumber);
    $ranLines[] = $lineNumber;
    ...
    try {
        ...
        echo 'Attempted to read outside of file!'.PHP_EOL;
        break;
    }
    ...
    // printf('%3s %+3d'.PHP_EOL, $operation, $value);
    switch ($operation) {
        case 'acc':
            $acc += $value;
            ++$lineNumber;
        ...
        case 'nop':
            ...
            break;
        case 'jmp':
            printf('Executed jmp: %d'.PHP_EOL, $lineNumber + 1);
            $lineNumber += $value;
...
printf('Value in the accumulator: %d'.PHP_EOL, $acc);
