...
$joltages[] = $joltages[count($joltages) - 1] + 3;

$differences = [];
for ($index = 0; $index < count($joltages) - 1; ++$index) {
    $current = $joltages[$index];
    $next = $joltages[$index + 1];
...
printf('Sum of 1-jolt differences times 3-jolt differences: %d'.PHP_EOL, $differences[1] * $differences[3]);
...
function determineLeaves(int $current, array $remaining)
{
    // echo 'Joltage ('.$current.'): '.implode(', ', $remaining).PHP_EOL;
    ...
    while ($diff >= 1 && $diff <= 3) {
        $sum += determineLeaves($next, $remaining);
 ...
 * Thanks to @https://github.com/DamienPirsy/AoC_2020/blob/master/10/day10.php
 ...
 * @return string
 */
function countBranches(array $numbers = [])
{
    $highest = $numbers[count($numbers)-1] - 3;
...
// echo 'Number of possible combinations: '.determineLeaves(array_shift($joltages), $joltages).PHP_EOL;
echo 'Number of possible combinations: '.countBranches($joltages).PHP_EOL;
