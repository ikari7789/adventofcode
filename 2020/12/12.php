...
function rotate($direction, $degrees, &$currentAxis, &$currentDirection)
{
    $modifier = null;
    if ($direction === 'R') {
        $modifier = 1;
    ...
    for ($turn = 0; $turn < $degrees / 90; ++$turn) {
        // East
        if ($currentAxis === 1 && $currentDirection === 1) {
            $currentAxis = 0;
            $currentDirection = -1 * $modifier;
        // South
        } elseif ($currentAxis === 0 && $currentDirection === -1) {
            $currentAxis = 1;
            $currentDirection = -1 * $modifier;
        // West
        } elseif ($currentAxis === 1 && $currentDirection === -1) {
            $currentAxis = 0;
            $currentDirection = 1 * $modifier;
        // North
        } elseif ($currentAxis === 0 && $currentDirection === 1) {
...
$currentAxis = 1; // 0 - x, 1 -y
$currentDirection = 1; // 1 = north/east, -1 = south/west
while (! $input->eof()) {
    $line = $input->fgets();
    $line = trim($line);
    ...
    preg_match('/^(?P<direction>[NSEWLRF])(?P<units>\d+)$/', $line, $matches);
    ...
    switch ($direction) {
        case 'L':
        case 'R':
            rotate($direction, $units, $currentAxis, $currentDirection);
            break;
    }
    ...
    switch ($direction) {
        ...
        case 'W':
            ...
            break;
        case 'F':
            $position[$currentAxis] += ($units * $currentDirection);
            break;
    }

    printf(
        '%-4s: {"units": %2d, "currentAxis": %d, "currentDirection": %d} (%d, %d)'.PHP_EOL,
        $line,
        $units,
        $currentAxis,
        $currentDirection,
...
printf('Manhattan direction: %d'.PHP_EOL, abs($position[0]) + abs($position[1]));
...
while (! $input->eof()) {
    ...
    switch ($direction) {
        ...
        case 'W':
            ...
            break;
        case 'F':
            $ship[$currentAxis] += ($units * $currentDirection);
            break;
    }
