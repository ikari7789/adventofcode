...
while (! $input->eof()) {
    ...
    for ($index = 0; $index < 7; ++$index) {
        ...
        $rows = array_slice($rows, $offset, count($rows) / 2);
    }
    $row = $rows[0];

    $columns = range(0, $columnCount - 1);
    for ($index = 7; $index < 10; ++$index) {
        switch ($line[$index]) {
            case 'L':
                ...
                break;
            case 'R';
                $offset = (count($columns) / 2);

                break;
        }
        $columns = array_slice($columns, $offset, count($columns) / 2);
    }
    $column = $columns[0];

    $seatId = ($row * 8) + $column;

    // printf('%s: row %d, column %d, seat ID %d'.PHP_EOL, $line, $row, $column, $seatId);
...
printf('Max seat ID: %d'.PHP_EOL, $maxSeatId);
 ...
 */
for ($index = 0; $index < count($seats) - 1; ++$index) {
    $previousSeat = isset($seats[$index - 1]);
    $seat = isset($seats[$index]);
    $nextSeat = isset($seats[$index + 1]);
    if (($previousSeat && $nextSeat) && ! $seat) {
        printf('ID not set %d'.PHP_EOL, $index);
    }
}
