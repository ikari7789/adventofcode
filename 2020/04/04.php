...
while (! $input->eof()) {
    ...
    // In between passports so let's evaluate what we have
    if (empty($line) || $input->eof()) {
        $passportParts = reduce($passportParts);
        ...
        $passports[] = $passportParts;

        $passportParts = [];
        continue;
    }

    $passportParts = array_merge(
        $passportParts,
        array_map(function($value) {
            ...
            return [$key => $passportValue];
        }, explode(' ', $line))
    );
}

echo 'Valid passports: '.count($passports).PHP_EOL;
...
function validate4DigitNumber($value, $min, $max)
{
    if (strlen($value) !== 4) {
        return false;
    }
...
function validateLength($value, $metricMinMax, $imperialMinMax)
{
    preg_match('/^(?P<length>\d+)(?P<system>cm|in)$/', $value, $matches);

    if (! isset($matches['length'], $matches['system'])) {
        return false;
    }
    ...
    switch ($matches['system']) {
        case 'cm':
            ...
            break;
        case 'in':
            $min = $imperialMinMax[0];
            $max = $imperialMinMax[1];
...
$badCount = 0;
for ($index = 0; $index < count($passports); ++$index) {
    $passport = $passports[$index];

    if (! validate4DigitNumber($passport['byr'], 1920, 2002)) {
        ++$badCount;
        continue;
    }

    if (! validate4DigitNumber($passport['iyr'], 2010, 2020)) {
        ++$badCount;
        continue;
    }

    if (! validate4DigitNumber($passport['eyr'], 2020, 2030)) {
        ++$badCount;
        continue;
    }

    if (! validateLength($passport['hgt'], [150, 193], [59, 76])) {
        ++$badCount;
        continue;
    }

    if (! validateHexColor($passport['hcl'])) {
        ++$badCount;
        continue;
    }
...
echo 'Valid passports: '.(count($passports) - $badCount);
