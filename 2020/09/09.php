...
// Test (cipher lookback: 5)
// Real (cipher lookback: 25)
...
function findSum($value, $cipherKeys) {
    for ($index1 = 0; $index1 < count($cipherKeys); ++$index1) {
        $value1 = $cipherKeys[$index1];

        for ($index2 = $index1 + 1; $index2 < count($cipherKeys); ++$index2) {
            $value2 = $cipherKeys[$index2];
...
while (! $input->eof()) {
    ...
    if (count($cipherKeys) < $cipherLookback + 1) {
        $cipherKeys[] = $value;
        continue;
    }

    if (count($cipherKeys) === $cipherLookback + 1) {
        array_shift($cipherKeys);
    }

    if (! findSum($value, $cipherKeys)) {
        printf('Value does not follow cipher rules: %d'.PHP_EOL, $value);
...
$cipherKeys = [];
$cipherLookback = 2;
while (array_sum($cipherKeys) !== $unmatchedValue) {
    $input->rewind();
    while (! $input->eof()) {
        $line = $input->fgets();
        $line = trim($line);
        ...
        if (array_sum($cipherKeys) === $unmatchedValue) {
            printf('Contiguous values found'.PHP_EOL);
...
sort($cipherKeys, SORT_NUMERIC);
print_r($cipherKeys);

printf('Encryption weakness: %d'.PHP_EOL, $cipherKeys[0] + $cipherKeys[count($cipherKeys) - 1]);
